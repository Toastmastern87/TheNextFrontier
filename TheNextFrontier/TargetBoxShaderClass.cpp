...
TargetBoxShaderClass::TargetBoxShaderClass()
{
	mVertexShader = 0;
	mPixelShader = 0;
	mLayout = 0;
...
TargetBoxShaderClass::TargetBoxShaderClass(const TargetBoxShaderClass& other)
{
}
...
bool TargetBoxShaderClass::Initialize(ID3D11Device *device, HWND hwnd)
{
	bool result;
...
void TargetBoxShaderClass::Shutdown()
{
	ShutdownShader();
...
bool TargetBoxShaderClass::Render(ID3D11DeviceContext *deviceContext, int indexCount, XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix, XMMATRIX positionMatrix, XMMATRIX scaleMatrix, XMMATRIX rotationMatrix, ID3D11ShaderResourceView* texture)
{
	bool result;
...
bool TargetBoxShaderClass::InitializeShader(ID3D11Device *device, HWND hwnd)
{
	HRESULT hResult;
	...
	hResult = D3DReadFileToBlob(L"../x64/Debug/TargetBox_vs.cso", &vertexShaderBuffer);
	if (FAILED(hResult))
	{
		return false;
	}
	...
	hResult = D3DReadFileToBlob(L"../x64/Debug/TargetBox_ps.cso", &pixelShaderBuffer);
	if (FAILED(hResult))
	{
		return false;
	}
...
void TargetBoxShaderClass::ShutdownShader()
{
	if (mConstantBuffer)
	{
...
bool TargetBoxShaderClass::SetShaderParameters(ID3D11DeviceContext *deviceContext, XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix, XMMATRIX positionMatrix, XMMATRIX scaleMatrix, XMMATRIX rotationMatrix, ID3D11ShaderResourceView* texture)
{
	HRESULT result;
...
void TargetBoxShaderClass::RenderShader(ID3D11DeviceContext *deviceContext, int indexCount)
{
